...
CircularBuffer<int,1000> allBlinksbuff;
CircularBuffer<double,1000> allDursbuff;
...
//          allSqrerrsbuff.push(sqrErr);
          ...
          if (sqrErr < sqrErrcutoff) {
            ...
            allBlinksbuff.push(peak_index-1); // for EXPERIMENT (tracking all classified blinks)
            candidateHandler += 1;
          } else {
            candidateHandler = 1;
          }
      ...
      if (!realBlinkbuff.isEmpty()) { // updating indices
        ...
        if (candidateHandler == 7) { // using a thresholding scheme to calculate the duration that IR reflectance above a certain threshold
          
         realBlinkbuff.shift();

         double thresh = measurement_min + 0.5*measurement_absdiff; // the actual fraction used for the threshold can be changed
         byte over = false;
         double sample_duration = 0;

         for (int i=0;i<201;i++) {
          if (over == false) {
            if (raw_measurement[i] > thresh) {
              over = true;
            }
          }else {
            sample_duration += 1;
            if (raw_measurement[i] < thresh && i > 100) {
              break;
            }
          }
         }

         sample_duration = sample_duration/604*1000;
         allDursbuff.push(sample_duration);
         candidateHandler = 1;
        }
      ...
      if (!allBlinksbuff.isEmpty()) {
        for (int i=0;i<allBlinksbuff.size();i++) {
          int update = allBlinksbuff.shift() + 1;
          allBlinksbuff.push(update);
        }
      }
   ...
   if (num_experiment_samples == buffMaxsize && readOuttoSerial == false) {
     ...
     Serial.println("LABEL,time,timer,blinks,durations,raw");
     Serial.println("RESETTIMER");
  
     for (int i=0;i<buffMaxsize;i++) {
      Serial.print("DATA,TIME,TIMER,");
      if (i < allBlinksbuff.size()) {
        Serial.print(buffMaxsize - allBlinksbuff[i]);
      }
      Serial.print(",");
      if (i < allDursbuff.size()) {
        Serial.print(allDursbuff[i]);
      }
      Serial.print(",");
  ...
  //     Serial.println("ch_0");
  //     for (int i=0; i<allBlinksbuff.size(); i++) {
  //       Serial.println(buffMaxsize - allBlinksbuff[i]);
  //     }
  //     
  //     Serial.println("ch_1");
  //     for (int i=0; i<allDursbuff.size(); i++) {
  //       Serial.println(allDursbuff[i]);
  //     }
  //     
  //     Serial.println("ch_2");
  //     for (int i=0; i<buffMaxsize; i++) {
